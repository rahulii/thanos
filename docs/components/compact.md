...
In both systems, series with the same labels are merged together. Merging samples is **naive**. It works by deduplicating samples within exactly the same timestamps. Otherwise samples are added in sorted by time order.
