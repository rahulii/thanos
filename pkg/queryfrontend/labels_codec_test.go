...
func TestLabelsCodec_MergeResponse(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			responses: []queryrange.Response{
				&ThanosSeriesResponse{Status: "success", Data: []labelpb.ZLabelSet{{Labels: []*labelpb.Label{{Name: "foo", Value: "bar"}}}}},
			},
			expectedResponse: &ThanosSeriesResponse{Status: "success", Data: []labelpb.ZLabelSet{{Labels: []*labelpb.Label{{Name: "foo", Value: "bar"}}}}},
		},
		{
